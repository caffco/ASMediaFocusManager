...
- (void)installOnView:(UIView *)view {
    ...
    tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:
                  @selector(handleFocusGesture:)];
    [view addGestureRecognizer:tapGesture];
    view.userInteractionEnabled = YES;

    UIPinchGestureRecognizer *pinchRecognizer = [[UIPinchGestureRecognizer alloc]
            initWithTarget:self action:@selector(handlePinchFocusGesture:)];
...
- (void)installDefocusActionOnFocusViewController:(ASMediaFocusController *)
    focusViewController {
    ...
    if (focusViewController.view) {
        if (self.isDefocusingWithTap) {
            ...
            tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:
                          @selector(handleDefocusGesture:)];
            [tapGesture requireGestureRecognizerToFail:
             focusViewController.doubleTapGesture];
...
- (void)setupAccessoryViewOnFocusViewController:(ASMediaFocusController *)
    focusViewController {
    UIButton *doneButton;
...
- (UIImage *)decodedImageWithImage:(UIImage *)image {
    ...
    CGRect imageRect = (CGRect) {
        .origin = CGPointZero, .size = imageSize
    };
...
- (CGRect)rectInsetsForRect:(CGRect)frame ratio:(CGFloat)ratio {
    ...
    resultFrame = CGRectMake(round(resultFrame.origin.x),
                             round(resultFrame.origin.y), round(resultFrame.size.width),
                             round(resultFrame.size.height));

    return resultFrame;
}

- (CGSize)sizeThatFitsInSize:(CGSize)boundingSize initialSize:
    (CGSize)initialSize {
    ...
    if (widthRatio < heightRatio) {
        fittingSize = CGSizeMake(boundingSize.width,
                                 floorf(initialSize.height * widthRatio));
    } else {
        fittingSize = CGSizeMake(floorf(initialSize.width * heightRatio),
                                 boundingSize.height);
    }
...
- (ASMediaFocusController *)focusViewControllerForView:(UIView *)mediaView {
    ...
    if (self.delegate
            && [self.delegate respondsToSelector:@selector(mediaFocusManager:
                    imageViewForView:)]) {
    ...
    viewController = [[ASMediaFocusController alloc] initWithNibName:nil bundle:
                      nil];
...
- (void)startFocusingView:(UIView *)mediaView {
    ...
    if (self.delegate
            && [self.delegate respondsToSelector:@selector(mediaFocusManagerWillAppear:)]) {
        [self.delegate mediaFocusManagerWillAppear:self];
    }

    self.mediaView = mediaView;
    parentViewController = [self.delegate parentViewControllerForMediaFocusManager:
                            self];
    ...
    center = [imageView.superview convertPoint:mediaView.center fromView:
              mediaView.superview];
    ...
    if (self.delegate
            && [self.delegate respondsToSelector:@selector(mediaFocusManager:
                    finalFrameForView:)]) {
        finalImageFrame = [self.delegate mediaFocusManager:self finalFrameForView:
                           mediaView];
    } else {
    ...
    if (imageView.contentMode == UIViewContentModeScaleAspectFill) {
        ...
        size = [self sizeThatFitsInSize:finalImageFrame.size initialSize:
                imageView.image.size];
        finalImageFrame.size = size;
        finalImageFrame.origin.x = (focusViewController.view.bounds.size.width -
                                    size.width) / 2;
        finalImageFrame.origin.y = (focusViewController.view.bounds.size.height -
                                    size.height) / 2;
    }
    ...
    duration = (self.elasticAnimation ? self.animationDuration *
                (1 - kAnimateElasticDurationRatio) : self.animationDuration);
    ...
    animations:^ {
        ...
        if (mediaView.layer.cornerRadius > 0) {
            [self animateCornerRadiusOfView:imageView withDuration:duration from:
            mediaView.layer.cornerRadius to:0.0f];
        }
    }
        ...
        completion: ^ (BOOL finished) {
            ...
            completion: ^ (BOOL finished) {
                ...
                completion: ^ (BOOL finished) {
                    ...
                    if (self.delegate
                            && [self.delegate respondsToSelector:@selector(mediaFocusManagerDidAppear:)]) {
...
- (void)animateCornerRadiusOfView:(UIView *)view withDuration:
    (NSTimeInterval)duration from:(float)initialValue to:(float)finalValue {
...
- (void)updateAnimatedView:(UIView *)view fromFrame:(CGRect)initialFrame
...
- (void)updateBoundsDuringAnimationWithElasticRatio:(CGFloat)ratio {
    ...
    frame = (self.elasticAnimation ? [self rectInsetsForRect:frame ratio:ratio] :
             frame);
    self.focusViewController.mainImageView.bounds = frame;
    [self updateAnimatedView:self.focusViewController.playerView fromFrame:
     initialFrame toFrame:frame];
}
...
- (void)endFocusing {
    ...
    if (self.mediaView.layer.cornerRadius > 0) {
        [self animateCornerRadiusOfView:contentView withDuration:duration from:0.0f to:
         self.mediaView.layer.cornerRadius];
    }
        ...
        completion: ^ (BOOL finished) {
            ...
            completion: ^ (BOOL finished) {
                ...
                completion: ^ (BOOL finished) {
                    ...
                    if (self.delegate
                            && [self.delegate respondsToSelector:@selector(mediaFocusManagerDidDisappear:
                                                                          )]) {
...
- (void)installSwipeGestureOnFocusView {
    ...
    swipeGesture = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:
                    @selector(handleDefocusBySwipeGesture:)];
...
- (void)handleDefocusBySwipeGesture:(UISwipeGestureRecognizer *)gesture {
    ...
    [self.focusViewController defocusWillStart];
    offset = (gesture.direction == UISwipeGestureRecognizerDirectionUp ?
              -kSwipeOffset : kSwipeOffset);
    contentView = self.focusViewController.mainImageView;
